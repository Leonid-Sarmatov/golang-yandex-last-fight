...
docker-compose up
```
Важно: команда ```docker-compose down``` удалит контейнеры, то есть данные в базе данных потеряются. Для остановки без удаления используйте запуск с последующим отклбчением через Ctrl+C в терминале. Во время запуска всей системы в терминале будем множество логов, когда система полностью готова к работе, оркестратор отправит в терминал изображение собаки из мема.
