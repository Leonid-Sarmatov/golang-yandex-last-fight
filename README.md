...
docker-compose up
```
Важно: команда ```docker-compose down``` удалит контейнеры, то есть данные в базе данных потеряются. Для остановки без удаления используйте запуск с последующим отклбчением через Ctrl+C в терминале. Скачивание всех образов и запуск системы займут какое то время, когда система полностью готова к работе, оркестратор отправит в терминал изображение собаки из мема.
...
```
Для тестирования отказов можно остановить (но не удалить!) один из работающих контейнеров
...
docker stop <id>
```
Так же в рамках тестирования я оставил уязвимость вычислителей: когда первый символ это минус, то есть в самом начале отрицательное число, то парсер паникует, и вычислители падают. Однако докер их автоматически поднимает, восстанавливая работоспособность системы.

# Проблемы и недоработки на текущий коммит(если успею пофиксить, это раздел уменьшится)
...
4. Вычислительный сервер запускает фиксированное количество экземпляров вычислителей(доделываю что бы количество прописывалось в конфигах)
